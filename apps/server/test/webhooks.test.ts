...
describe("GitHub webhook routes", () => {
  it("reports duplicates detected during ingest", async () => {
    ...
    expect(first.json<{ accepted: boolean; duplicate: boolean }>()).toMatchObject({
      accepted: true,
      duplicate: false
    });
    expect(duplicate.statusCode).toBe(202);
    expect(duplicate.json<{ accepted: boolean; duplicate: boolean; items: unknown[] }>()).toMatchObject({
      accepted: false,
      duplicate: true,
      items: []
    });
