...
describe("github-app-token CLI strict validation", () => {
  it("rejects partially numeric installation ids before parsing the private key", async () => {
    const result = await runTokenCli({
      GITHUB_APP_ID: "123",
      GITHUB_PRIVATE_KEY: "not-a-private-key",
      GITHUB_INSTALLATION_ID: "123abc"
    });
  ...
  it("rejects malformed repository selectors before parsing the private key", async () => {
    const result = await runTokenCli({
      GITHUB_APP_ID: "123",
      GITHUB_PRIVATE_KEY: "not-a-private-key",
    ...
    expect(result.code).not.toBe(0);
    expect(result.stdout).toBe("");
    expect(result.stderr).toContain("--repository must use owner/name format.");
    expect(result.stderr).not.toContain("private");
  });
});
