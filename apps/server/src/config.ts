...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): AppConfig {
  ...
  const ai: AppConfig["ai"] = {
    ...
    maxInputChars: parseInteger(env.AI_MAX_INPUT_CHARS, 12000, { min: 1 })
  };
  ...
  return {
    ...
    port: parseInteger(env.PORT, 3000, { min: 0 }),
    ...
    scanners: {
      ...
      timeoutMs: parseInteger(env.SCANNER_TIMEOUT_MS, 120000, { min: 1 })
    },
...
function parseInteger(value: string | undefined, fallback: number, options: { min: number }): number {
