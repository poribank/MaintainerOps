...
Scorecard uses `scorecard --repo=github.com/owner/name --format=json`. OSV Scanner uses `osv-scanner scan source --format json -r <path>`, limited to paths inside the MaintainerOps workspace.
