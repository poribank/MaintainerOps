...
const repository = parseRepository(args.repository ?? process.env.GITHUB_REPOSITORY ?? "poribank/MaintainerOps");
if (!repository) {
...
function renderMarkdown(data) {
  ...
  const lines = [
    ...
    "## Repository",
    "",
    `- URL: ${inlineMarkdown(repo.htmlUrl)}`,
    `- Visibility: ${repo.private ? "private" : "public"}`,
    `- Default branch: ${inlineMarkdown(repo.defaultBranch)}`,
    `- Stars: ${repo.stars}`,
    `- Forks: ${repo.forks}`,
    `- Open issues: ${data.openIssues}`,
    `- GitHub open issues count: ${repo.githubOpenIssuesCount}`,
    `- License: ${inlineMarkdown(repo.license ?? "unknown")}`,
  ...
  if (latestRelease) {
    lines.push(
      `- Latest release: ${inlineMarkdown(latestRelease.tagName)}`,
  ...
  if (workflowUnavailable) {
    lines.push(`- Unavailable: ${inlineMarkdown(workflowUnavailable)}`);
  } else if (workflowRuns.length === 0) {
    lines.push("- No workflow runs returned by the GitHub Actions API.");
  } else {
    for (const run of workflowRuns) {
      lines.push(
        `- ${inlineMarkdown(run.name)}: ${inlineMarkdown(run.status)}/${inlineMarkdown(
          run.conclusion ?? "pending"
        )} on ${inlineMarkdown(run.headBranch)} at ${run.createdAt}`
      );
    }
  }
